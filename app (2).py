...
# 차트 공통 색상 (분석2 스타일 — 파란/초록 계열)
COLOR_LOW  = '#636EFA'   # 저소득
COLOR_MID  = '#EF553B'   # 중간
COLOR_HIGH = '#00CC96'   # 고소득
ORDER = ['저소득', '중간', '고소득']
CMAP = {'저소득': COLOR_LOW, '중간': COLOR_MID, '고소득': COLOR_HIGH}
 
def make_bar(df, x_col, y_col, title, ymax=None):
    ...
    yrange = [0, (ymax or df[y_col].max()) * 1.3]
    fig.update_layout(
        title=title,
        yaxis=dict(showgrid=True, gridcolor='#eee', range=yrange),
...
with st.expander("사용한 SQL 및 해석 보기"):
